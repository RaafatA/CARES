...
Key Updates:
1. Primary Wheat Grain Yield Metric: Thousand-Kernel Weight (TKW, g) replaces Shoot DW in Figure 2A.
2. Growing Season Alignment: 18 December to late April (150 days on average).
   - Fig 1 x-axis: Days After Sowing (DAS 0 to ~150; 18-Dec sowing to late April harvest).
3. Biomass Units: Standardized as Weight (g/hill) for Berseem forage and Total System Biomass.
4. Dual Export: Saves all publication figures in BOTH high-res PNG (300 DPI) and vector SVG formats.
5. Experimental Milestones: 15 cm plant spacing, 50% deficit program, and 3-cut forage cycles.
"""
...
class IntercroppingExploratoryVisualizer:
    def __init__(self, processed_data_path='data/wheat_berseem_real_processed.csv', daily_weather_path=None, output_dir='figures'):
        ...
        if resolved_weather_path:
            ...
            def assign_season(d):
                if d >= pd.Timestamp('2021-12-18') and d <= pd.Timestamp('2022-05-17'):
                    return 1
                elif d >= pd.Timestamp('2022-12-18') and d <= pd.Timestamp('2023-05-17'):
                    return 2
                else:
                    return np.nan
    ...
    # =========================================================================
    # FIGURE 2: MULTI-FACTORIAL GRAIN QUALITY (TKW), FORAGE & SYSTEM BIOMASS
    # =========================================================================
    def plot_fig2_biomass_yield_performance(self):
        """Figure 2: Faceted Bar/Box Plots of Wheat TKW, Berseem Forage, and Total System Biomass."""
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(14, 11), dpi=300)
        
        # 1. Wheat Thousand-Kernel Weight (TKW, g) by Water Quality and Field Capacity
        w_sub = self.df[self.df['Cropping_system'].isin(['MW', 'WBI'])].copy()
        sns.barplot(data=w_sub, x='Water_quality', y='TKW', hue='Field_capacity', 
                    palette=['#2ca02c', '#d62728'], ax=ax1, errorbar='se', capsize=0.1, edgecolor='black')
        ax1.set_title('(A) Wheat Thousand-Kernel Weight (TKW, g)', fontsize=12, fontweight='bold', loc='left')
        ax1.set_xlabel('Irrigation Water Quality', fontsize=11, fontweight='bold')
        ax1.set_ylabel('Thousand-Kernel Weight (TKW, g)', fontsize=11, fontweight='bold')
        ax1.set_ylim(40, 75)
        ax1.grid(axis='y')
        ax1.legend(title='Field Capacity', frameon=True, fontsize=9)
        ...
        ax2.set_title('(B) Berseem Cumulative Forage Dry Matter (g/hill)', fontsize=12, fontweight='bold', loc='left')
        ...
        ax2.grid(axis='y')
        ax2.legend(title='Field Capacity', frameon=True, fontsize=9)
        ...
        ax3.grid(axis='y')
        ax3.legend(title='Field Capacity', frameon=True, fontsize=9)
        ...
        ax4.grid(axis='y')
        ax4.legend(title='Season', frameon=True, fontsize=9)

        plt.suptitle('Figure 2: Wheat Grain Yield Quality (TKW), Forage Production, and Total System Biomass Responses', fontsize=14, fontweight='bold', y=0.995)
        plt.tight_layout()
    ...
    def plot_fig6_correlation_heatmap(self):
        ...
        selected_traits = [
            'TKW', 'Wheat_Shoot_DW_g_hill', 'Berseem_Dry_Weight_g_hill', 'Total_System_Biomass_g_hill',
            'pLER_Wheat', 'pLER_Berseem', 'LER_Total', 'ATER', 'CR_Wheat', 'IWUE_kg_m3', 'SPI',
            'Wheat_PH_cm', 'Wheat_SPAD', 'LAI', 'Tillers_per_plant', 'Root_DW',
        ...
        plt.title('Figure 6: Clustered Pearson Correlation Matrix linking Grain Quality (TKW), Morphological Traits, & System Efficiencies', 
    ...
    def plot_fig8_pca_multivariate_analysis(self):
        ...
        pca_features = [
            'TKW', 'Wheat_Shoot_DW_g_hill', 'Berseem_Dry_Weight_g_hill', 'Total_System_Biomass_g_hill', 'LER_Total', 'IWUE_kg_m3',
            'Wheat_PH_cm', 'Wheat_SPAD', 'LAI', 'Tillers_per_plant', 'Root_DW',
